...
ESM `.js` skill file per agent into `~/.workforce-agi/skills/` (or the
...
but without requiring users to modify the runtime source.

Usage:
    export_to_workforce_agi.py                # export into ~/.workforce-agi/skills/
    export_to_workforce_agi.py --dry-run      # print what would be written
    export_to_workforce_agi.py --out-dir ./x  # write into ./x instead
"""
...
#
# Layout post-rebrand:
#   <repo>/scripts/export_to_workforce_agi.py   ← __file__
#   <repo>/agents/{_shared,foundation,workforce}/*.md
#
# parents[1] resolves to <repo>. The legacy nested `foundation/agents/`
# path was removed when the project rebranded — keep the resolution
# anchored to the repo root so the exporter survives further reshuffles.
...
SKILL_TEMPLATE = """\
// Auto-generated by scripts/export_to_workforce_agi.py
// Source: {source_rel}
...
def default_out_dir() -> Path:
    override = os.environ.get("WORKFORCE_AGI_DATA_DIR")
    base = Path(override) if override else Path.home() / ".workforce-agi"
    return base / "skills"


def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Export Foundation agent markdown files as workforce-agi .js skill modules.",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=None,
        help="Directory to write .js skill files into (default: ~/.workforce-agi/skills/).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if not agents:
        print(f"No Foundation agents found under {AGENTS_ROOT}.", file=sys.stderr)
        return 1
